...
class User(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    carrier_id: Mapped[Optional[uuid.UUID]] = mapped_column(
        UUID(as_uuid=True), ForeignKey("carriers.id", ondelete="SET NULL"),
        nullable=True, index=True,
    )
    ...
    # Relationships
    supplier: Mapped[Optional["Supplier"]] = relationship("Supplier", back_populates="users", foreign_keys=[supplier_id])
    carrier: Mapped[Optional["Carrier"]] = relationship("Carrier", back_populates="users", foreign_keys=[carrier_id])
...
class Carrier(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    # Relationships
    users: Mapped[list["User"]] = relationship("User", back_populates="carrier",
                                                foreign_keys="User.carrier_id")
    contracts: Mapped[list["Contract"]] = relationship("Contract", back_populates="carrier")
...
class Supplier(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    name: Mapped[str] = mapped_column(String(256), nullable=False)
    tax_id: Mapped[Optional[str]] = mapped_column(String(32), nullable=True, comment="EIN or SSN (masked in UI)")
    is_active: Mapped[bool] = mapped_column(Boolean, nullable=False, default=True, server_default="true")
    ...
    mapping_rules: Mapped[list["MappingRule"]] = relationship("MappingRule", back_populates="supplier")
...
class Contract(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    __table_args__ = (
        UniqueConstraint("supplier_id", "carrier_id", "effective_from", name="uq_contract_effective"),
    )
    ...
    name: Mapped[str] = mapped_column(String(256), nullable=False, comment="e.g. 'ACME IME Services Agreement 2024'")
    effective_from: Mapped[date] = mapped_column(Date, nullable=False)
    effective_to: Mapped[Optional[date]] = mapped_column(Date, nullable=True, comment="NULL = still active")

    geography_scope: Mapped[str] = mapped_column(
        String(16), nullable=False, default=GeographyScope.NATIONAL,
    ...
    rate_cards: Mapped[list["RateCard"]] = relationship("RateCard", back_populates="contract",
                                                         cascade="all, delete-orphan")
    guidelines: Mapped[list["Guideline"]] = relationship("Guideline", back_populates="contract",
                                                          cascade="all, delete-orphan")
...
class RateCard(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    taxonomy_code: Mapped[str] = mapped_column(
        String(64), ForeignKey("taxonomy_items.code", ondelete="RESTRICT"), nullable=False, index=True,
    )
...
class Guideline(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    taxonomy_code: Mapped[Optional[str]] = mapped_column(
        String(64), ForeignKey("taxonomy_items.code", ondelete="RESTRICT"), nullable=True, index=True,
    )
    ...
    def __repr__(self) -> str:
        return f"<Guideline rule_type={self.rule_type!r} taxonomy={self.taxonomy_code!r}>"
