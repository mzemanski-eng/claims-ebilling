...
class ConfirmedBy:
    ...
    CARRIER_CONFIRMED = "CARRIER_CONFIRMED"   # carrier reviewed and accepted system mapping
...
class MappingRule(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    match_type: Mapped[str] = mapped_column(
        ...
        comment="exact_code | regex_pattern | keyword_set"
    )
    ...
    confirmed_by: Mapped[str] = mapped_column(
        String(32), nullable=False, default=ConfirmedBy.SYSTEM,
    ...
    # Relationships
    supplier: Mapped[Optional["Supplier"]] = relationship("Supplier", back_populates="mapping_rules")
    confirmed_by_user: Mapped[Optional["User"]] = relationship("User")
    superseded_rule: Mapped[Optional["MappingRule"]] = relationship(
        "MappingRule", remote_side="MappingRule.id", foreign_keys=[supersedes_rule_id],
    )
