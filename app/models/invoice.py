...
class SubmissionStatus:
    ...
    ALL = [
        DRAFT, SUBMITTED, PROCESSING, REVIEW_REQUIRED,
        SUPPLIER_RESPONDED, PENDING_CARRIER_REVIEW, CARRIER_REVIEWING,
        APPROVED, DISPUTED, EXPORTED, WITHDRAWN,
    ]
...
class Invoice(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    status: Mapped[str] = mapped_column(
        String(32), nullable=False, default=SubmissionStatus.DRAFT, index=True,
    )
    ...
    versions: Mapped[list["InvoiceVersion"]] = relationship(
        "InvoiceVersion", back_populates="invoice", cascade="all, delete-orphan",
    ...
    def __repr__(self) -> str:
        return f"<Invoice invoice_number={self.invoice_number!r} status={self.status!r}>"
...
class InvoiceVersion(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    extraction_artifacts: Mapped[list["RawExtractionArtifact"]] = relationship(
        "RawExtractionArtifact", back_populates="invoice_version", cascade="all, delete-orphan",
    )
...
class LineItem(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    # ── Status ───────────────────────────────────────────────────────────────
    status: Mapped[str] = mapped_column(
        String(32), nullable=False, default=LineItemStatus.PENDING, index=True,
    )

    # ── Raw extraction ────────────────────────────────────────────────────────
    raw_description: Mapped[str] = mapped_column(Text, nullable=False)
    raw_code: Mapped[Optional[str]] = mapped_column(String(64), nullable=True,
                                                     comment="Supplier's own billing code, if any")
    raw_amount: Mapped[Decimal] = mapped_column(Numeric(12, 2), nullable=False)
    raw_quantity: Mapped[Decimal] = mapped_column(Numeric(10, 4), nullable=False, default=1)
    ...
    taxonomy_code: Mapped[Optional[str]] = mapped_column(
        String(64), ForeignKey("taxonomy_items.code", ondelete="RESTRICT"),
    ...
    mapping_rule_id: Mapped[Optional[uuid.UUID]] = mapped_column(
        UUID(as_uuid=True), ForeignKey("mapping_rules.id", ondelete="SET NULL"),
        nullable=True,
    )

    # ── Validation output ─────────────────────────────────────────────────────
    mapped_rate: Mapped[Optional[Decimal]] = mapped_column(Numeric(12, 4), nullable=True)
    ...
    validation_results: Mapped[list["ValidationResult"]] = relationship(
        "ValidationResult", back_populates="line_item", cascade="all, delete-orphan",
    )
    exceptions: Mapped[list["ExceptionRecord"]] = relationship(
        "ExceptionRecord", back_populates="line_item", cascade="all, delete-orphan",
    )
...
class RawExtractionArtifact(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    invoice_version_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("invoice_versions.id", ondelete="CASCADE"),
        nullable=False, index=True,
    )

    page_number: Mapped[Optional[int]] = mapped_column(Integer, nullable=True,
                                                        comment="Page number for PDFs; NULL for CSV rows")
    raw_text: Mapped[str] = mapped_column(Text, nullable=False)
    extraction_method: Mapped[str] = mapped_column(
        String(32), nullable=False,
        comment="pdfplumber | csv | manual"
    )
    ...
    invoice_version: Mapped["InvoiceVersion"] = relationship(
        "InvoiceVersion", back_populates="extraction_artifacts",
    )
