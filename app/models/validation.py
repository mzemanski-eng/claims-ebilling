...
class ValidationResult(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    severity: Mapped[str] = mapped_column(
        String(16), nullable=False, default=ValidationSeverity.ERROR,
    ...
    required_action: Mapped[str] = mapped_column(
        String(32), nullable=False, default=RequiredAction.NONE,
        comment="NONE | REUPLOAD | ATTACH_DOC | REQUEST_RECLASSIFICATION | ACCEPT_REDUCTION"
    )

    # Relationships
    line_item: Mapped["LineItem"] = relationship("LineItem", back_populates="validation_results")
...
class ExceptionRecord(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    validation_result_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("validation_results.id", ondelete="RESTRICT"),
        nullable=False,
    )

    # ── Lifecycle ─────────────────────────────────────────────────────────────
    status: Mapped[str] = mapped_column(
        String(32), nullable=False, default=ExceptionStatus.OPEN, index=True,
    ...
    supporting_doc_path: Mapped[Optional[str]] = mapped_column(String(512), nullable=True)
    ...
    # Relationships
    line_item: Mapped["LineItem"] = relationship("LineItem", back_populates="exceptions")
    validation_result: Mapped["ValidationResult"] = relationship("ValidationResult")

    def __repr__(self) -> str:
        return f"<ExceptionRecord status={self.status!r} line_item_id={self.line_item_id}>"
