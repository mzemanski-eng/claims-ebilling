...
class ClassificationResult:
    ...
    match_type: Optional[str]            # exact_code | regex_pattern | keyword_set | heuristic | None
    matched_rule_id: Optional[str]       # MappingRule.id if DB-backed; None for built-in heuristics
...
BUILTIN_RULES: list[tuple[str, str, str, str, float]] = [
    ...
    ("keyword_set", "independent medical examination",  "IME.PHY_EXAM.PROF_FEE",         "PROF_FEE",          0.80),
    ...
    ("regex_pattern", r"\bindependent medical\b",       "IME.PHY_EXAM.PROF_FEE",         "PROF_FEE",          0.80),
    ("keyword_set", "multi.specialty,panel,ime",        "IME.MULTI_SPECIALTY.PROF_FEE",  "PROF_FEE",          0.80),
    ("keyword_set", "multi-specialty,ime",              "IME.MULTI_SPECIALTY.PROF_FEE",  "PROF_FEE",          0.80),
    ("keyword_set", "records review,no exam",           "IME.RECORDS_REVIEW.PROF_FEE",   "PROF_FEE",          0.85),
    ("keyword_set", "file review,no exam",              "IME.RECORDS_REVIEW.PROF_FEE",   "PROF_FEE",          0.82),
    ("regex_pattern", r"records?\s+review.*no.?exam",   "IME.RECORDS_REVIEW.PROF_FEE",   "PROF_FEE",          0.85),
    ...
    ("regex_pattern", r"\bpeer.?review\b",              "IME.PEER_REVIEW.PROF_FEE",      "PROF_FEE",          0.88),
    ("keyword_set", "cancellation,fee",                 "IME.CANCELLATION.CANCEL_FEE",   "CANCEL_FEE",        0.90),
    ...
    ("keyword_set", "scheduling,fee",                   "IME.ADMIN.SCHEDULING_FEE",      "SCHEDULING_FEE",    0.80),
    ("keyword_set", "admin,scheduling",                 "IME.ADMIN.SCHEDULING_FEE",      "SCHEDULING_FEE",    0.78),

    # ── ENG ──────────────────────────────────────────────────────────────────
    ("keyword_set", "property,inspection,engineer",     "ENG.PROPERTY_INSPECT.PROF_FEE", "PROF_FEE",          0.82),
    ("keyword_set", "cause,origin",                     "ENG.CAUSE_ORIGIN.PROF_FEE",     "PROF_FEE",          0.90),
    ("regex_pattern", r"cause\s+(&|and)\s+origin",      "ENG.CAUSE_ORIGIN.PROF_FEE",     "PROF_FEE",          0.92),
    ("keyword_set", "structural,assessment",            "ENG.STRUCTURAL_ASSESS.PROF_FEE","PROF_FEE",          0.88),
    ("keyword_set", "expert,report,engineer",           "ENG.EXPERT_REPORT.PROF_FEE",    "PROF_FEE",          0.80),
    ("keyword_set", "testimony,deposition",             "ENG.TESTIMONY_DEPO.PROF_FEE",   "PROF_FEE",          0.88),
    ("keyword_set", "supplemental,inspection",          "ENG.SUPPLEMENTAL_INSPECT.PROF_FEE","PROF_FEE",       0.82),

    # ── IA ───────────────────────────────────────────────────────────────────
    ("keyword_set", "field,adjust",                     "IA.FIELD_ASSIGN.PROF_FEE",      "PROF_FEE",          0.82),
    ("keyword_set", "field adjusting,daily rate",       "IA.FIELD_ASSIGN.PROF_FEE",      "PROF_FEE",          0.88),
    ("keyword_set", "desk,assignment,adjust",           "IA.DESK_ASSIGN.PROF_FEE",       "PROF_FEE",          0.82),
    ...
    ("keyword_set", "catastrophe,assignment",           "IA.CAT_ASSIGN.PROF_FEE",        "PROF_FEE",          0.88),
    ("regex_pattern", r"\bcat\s+(assign|deployment|daily)\b", "IA.CAT_ASSIGN.PROF_FEE", "PROF_FEE",          0.85),
    ("keyword_set", "photo,documentation",              "IA.PHOTO_DOC.PROF_FEE",         "PROF_FEE",          0.88),
    ("keyword_set", "supplement,handling",              "IA.SUPPLEMENT_HANDLING.PROF_FEE","PROF_FEE",         0.88),
    ...
    ("keyword_set", "background,asset",                 "INV.BACKGROUND_ASSET.PROF_FEE", "PROF_FEE",          0.85),
    ...
    # ── REC ──────────────────────────────────────────────────────────────────
    ("keyword_set", "medical,records,retrieval",        "REC.MED_RECORDS.RETRIEVAL_FEE", "RETRIEVAL_FEE",     0.88),
    ("keyword_set", "medical records,request",          "REC.MED_RECORDS.RETRIEVAL_FEE", "RETRIEVAL_FEE",     0.85),
    ("keyword_set", "copy,per page,records",            "REC.MED_RECORDS.COPY_REPRO",    "COPY_REPRO",        0.82),
    ("keyword_set", "rush,records",                     "REC.MED_RECORDS.RUSH_PREMIUM",  "RUSH_PREMIUM",      0.85),
    ("keyword_set", "certified,copy",                   "REC.MED_RECORDS.CERT_COPY_FEE", "CERT_COPY_FEE",     0.85),
    ("keyword_set", "employment,records",               "REC.EMPLOYMENT_RECORDS.RETRIEVAL_FEE","RETRIEVAL_FEE",0.88),
    ("keyword_set", "court,records",                    "REC.LEGAL_RECORDS.RETRIEVAL_FEE","RETRIEVAL_FEE",    0.85),
    ("keyword_set", "police,report",                    "REC.LEGAL_RECORDS.RETRIEVAL_FEE","RETRIEVAL_FEE",    0.82),
    ...
    ("keyword_set", "airfare",                          "IME.PHY_EXAM.TRAVEL_TRANSPORT", "TRAVEL_TRANSPORT",  0.65),
    ...
    ("keyword_set", "meals,per diem",                   "IME.PHY_EXAM.TRAVEL_MEALS",     "TRAVEL_MEALS",      0.65),
    ("keyword_set", "pass.through",                     "XDOMAIN.PASS_THROUGH.THIRD_PARTY_COST","THIRD_PARTY_COST",0.70),
]
    ...
    for match_type, pattern, taxonomy_code, billing_component, weight in BUILTIN_RULES:
        if match_type == "regex_pattern":
            try:
                ...
                continue
            compiled.append((match_type, pattern, rx, taxonomy_code, billing_component, weight))
        elif match_type == "keyword_set":
            keywords = [k.strip().lower() for k in re.split(r"[,|]", pattern)]
            compiled.append((match_type, pattern, keywords, taxonomy_code, billing_component, weight))
        else:
            compiled.append((match_type, pattern, None, taxonomy_code, billing_component, weight))
    return compiled
...
def classify_with_builtin_rules(
    ...
    best: Optional[tuple] = None  # (weight, taxonomy_code, billing_component, match_type, pattern, explanation)
    ...
    for rule in get_compiled_rules():
        ...
        if match_type == "keyword_set":
            ...
            if all(kw in desc_lower or kw.replace(".", "").replace("-", "") in desc_lower
                   for kw in keywords):
                if best is None or weight > best[0]:
                    best = (weight, taxonomy_code, billing_component, match_type, pattern_str,
    ...
    weight, taxonomy_code, billing_component, match_type, pattern_str, explanation = best
